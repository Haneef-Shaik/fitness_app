...
| Secrets | `expo-secure-store` ~14.0.1 *(already installed)* | [D10](08-PROJECT-CHARTER.md#6-decision-log) — the sign-in in the device keychain: Supabase's session since 26 Sep ([14-SUPABASE.md](14-SUPABASE.md)) |
...
| Log / delete a **body measurement** | `body(*)`, `dashboard(*)`, `goals(*)`, `nutritionAnalytics(*)` | B-01 carries the body card, and a goal's progress is measured against the latest weigh-in. The diary and `analytics` are deliberately absent — stepping on a scale changes neither — but H-14 draws weight under intake (G10) |
