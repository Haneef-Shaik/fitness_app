...
| D10 | Refresh token in the device keychain, not an httpOnly cookie | 21 Sep | Native apps cannot use cookies; rotation + family revocation replaces that protection. *Still holds under D30:* the keychain now holds Supabase's session, and Supabase's refresh-token reuse detection does the revoking |
