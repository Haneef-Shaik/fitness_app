...
| **H6.2** | Analytics read model | Aggregations that do not scan every set on every request | G6 | ✅ 23 Sep — **no migration needed, and that is evidenced rather than asserted**: `ix_sessions_user_local_date` is partial on `status='completed'` and keyed `(user_id, local_date)`, which is exactly the filter; sets load by `session_exercise_id`, covered by `uq_set_index`. Two tests assert the query PLAN uses the index and never a `Seq Scan`, so widening the filter later fails the build |
| **H7.1** | Food resolver interface | Provider-agnostic — **Q1 can be answered late without a rewrite** | G7 | ✅ 23 Sep — `app/food/resolver.py` is a `Protocol` with `search(query) -> list[Candidate]` and `resolve(ref) -> FoodRef`. `InternalCatalogResolver` implements it over **22 seeded foods**; a test double is substituted in the suite, so "substitutable" is demonstrated rather than asserted. `_resolver()` in `app/api/routes/nutrition.py` is the **only** place a concrete resolver is named — no route and no screen imports one. Q1 remains open and G7 shipped without it |
| **H7.2** | Meal aggregate | Denormalised macros; only `confirmed = true` reaches analytics | G7 | ✅ 23 Sep — `meal_items` carry absolute macros frozen at write; `foods` carry per 100 g. Editing a food after logging leaves the meal **byte identical** (asserted). `confirmed` is filtered in ONE place — `app.domain.nutrition.day_totals` — and no route sums anything itself. Mutation-checked: summing outside the domain fails AC-07 and three others; counting unconfirmed items fails the I2/D5 test |
