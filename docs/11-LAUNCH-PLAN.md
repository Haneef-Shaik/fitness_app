...
|---|----------|----------------|--------|
| L1 | **Database host** | Supabase (Pro) as managed Postgres **and** Storage for photos — ✅ decided. **26 Sep, owner: Supabase Auth too**, with Google and Apple sign-in and Storage thumbnails ([14-SUPABASE.md](14-SUPABASE.md)). The FitLog API stays the only way to the data — no RLS, no Data API, no direct client writes (they would bypass the outbox and the server's authoritative recompute) | Phase 4 |
| L2 | **API + worker host** | Railway, Fly.io or Render — two processes (API, worker), HTTPS, same region as the database | Phase 4 |
| L3 | **Transactional email** | Resend or Postmark — as **Supabase Auth's SMTP** (docs/12 §2.1): Supabase sends every auth email | A-03, A-05, K-02 |
...
- [ ] **A-05 Forgot / Reset Password** — *Build side done*, **on Supabase Auth since 26 Sep** (docs/14): "Forgot password?" answers the same whether or not the address has an account; the email carries a link (PKCE: it works only on the phone that asked) **and a 6-digit code** for an email read anywhere else ("I have a code"); the new password signs every other device out, and Supabase emails a "password changed" notice · Evidence: `forgotPassword.test.tsx`, `resetPassword.test.tsx`, `supabaseAuth.test.ts`, and the loop through the local stack's inbox (link, code, wrong code, notice) · **Left for the owner (L3):** the project's SMTP (Resend), a verified sending domain and the templates (docs/12 §2.1); then the loop through a real inbox
- [x] **A-06 Verify Email** — **changed 26 Sep (docs/14 S8): a password account confirms its address before its first sign-in.** With Supabase linking sign-ins that share a verified address, an unconfirmed sign-up would let someone pre-register a victim's address and be linked into their later Google sign-in. Sign-up shows "Check your email" with a resend; log in offers the link again to an unconfirmed account; the link opens the app signed in (`app/auth/callback.tsx`). Google and Apple addresses are verified by the provider · Evidence: `registerScreen.test.tsx`, `loginScreen.test.tsx`, `authCallback.test.tsx`
- [x] **K-02 Account & Security** — `/settings/security`, on Supabase Auth since 26 Sep: change password (asks for the current one, keeps this device, signs out the rest), change email (the password first; changes once links at **both** addresses are opened — Supabase's secure email change — and the old address gets a notice), sign out other devices (the API refuses their tokens at once, S3). A Google or Apple account sees how it signed in instead of forms that cannot work — *Build* · Evidence: `securityScreen.test.tsx`, `supabaseAuth.test.ts`, `test_supabase_auth.py`
...
- [x] **Rate limiting** — Postgres-backed fixed windows (shared by every API instance, `m16`), per address and per account, with `Retry-After`: account deletion (app and web), AI, imports, feedback. **Since 26 Sep, sign-in, sign-up, refresh and auth emails are limited by Supabase Auth** (its per-IP and per-hour limits; docs/12 §2.1) — FitLog's own login limits went with its login. `X-Forwarded-For` is trusted only for `TRUSTED_PROXY_COUNT` hops. Sign-in also costs the same time for an unknown email as for a wrong password (no enumeration by timing) — *Build* · Evidence: `tests/test_rate_limits.py` (the 11th login in a minute is a 429; windows reset; every protected route listed and driven to its 429), `tests/test_auth.py` (+1)
...
- [x] **Security review** of the whole launch branch (26 Sep) — *Build*. Two earlier reviews (recovery/security; privacy) found and fixed an email-change takeover, a revoke race, limiter pool exhaustion, a stranger blocking in-app deletion, an upload-key traversal and photo deletion wiping other users' files. The final whole-branch review found **0 critical, 2 high, 3 medium, 8 low — all fixed, each with a test**:
  ...
  - **Left for the owner (L8):** email links return to the app on the `fitlog://` scheme, which another Android app could claim. Since the move to Supabase Auth that app would get a PKCE **code**, useless without the verifier kept on the phone that asked; https App Links / Universal Links still need the production domain
...
- [ ] Production secrets in the host's secret store: `DATABASE_URL`, `SUPABASE_URL` and `SUPABASE_SECRET_KEY`, `UPLOAD_SIGNING_SECRET`, `AI_PROVIDER=anthropic`, `AI_API_KEY`, storage keys (docs/12 §4.1) — *You*
...
### Both platforms
- [x] **Upgrade Expo SDK** 52 → **57** (React Native 0.76 → **0.86**, React 18 → **19.2**, expo-router 4 → 57), one SDK at a time with `expo-doctor` clean at each step; **`targetSdkVersion` 36 / `compileSdk` 36**, read from the built APK. Breaking changes handled: `expo-file-system` moved to `/legacy` (upload, prefs, export, import), notification display flags split, Metro's monorepo defaults, `StyleSheet.absoluteFill`; new dependencies re-aligned (`@sentry/react-native` 7.11, `expo-document-picker` 57). The same upgrade carries react-native#48547 — **#15b verified on the emulator's hardware keyboard (26 Sep) and the focus bridge it needed removed**; #28 still needs the phone — *Build* · Evidence: 1,221 client tests + coverage gate green, `tsc` clean, `expo-doctor` 21/21, `pnpm audit` 0 high / 0 critical, and a **release build on SDK 57** (below). Tap → set re-measured on the phone on SDK 57: **p95 80.7 ms** (budget 100), after fixing a G11 regression that had it at 164.6 ms
...
- [ ] Store assets: phone screenshots (both stores' sizes), feature graphic, short and full descriptions, keywords — *You* (screenshots can come from the Maestro flows — *Build*) · **Build side done for Android:** the Play phone set (six, 1080×1920) is in `docs/store/screenshots/android/`, taken from the release build by `scripts/store-screenshots.sh` on a seeded account; descriptions drafted in [13-STORE-LISTING.md](13-STORE-LISTING.md). Taking them found and fixed four defects — every line chart drawn from zero (a 3 kg cut was a flat line), weigh-in times shown in UTC, no visible selected range on the weight screen, and the AI review showing the model's macros for a matched food while confirm saved the food's — plus an unattended match onto a restaurant menu item. Left: the feature graphic (real icon, L8) and the iPhone sets (iOS build)
...
- **#15b** Shift+Tab cannot enter a text field — **closed 26 Sep** (SDK 57, verified on the emulator). **#28** TalkBack formatting spans — still dated 2026-12-15, needs the phone
